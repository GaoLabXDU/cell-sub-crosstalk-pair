'''
Author: Chenxing Zhang, Yuxuan Hu and Lin Gao
Date: 2023-03-08
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lrPath', required=True, help='the path of Ligand-Receptor dataset')
    parser.add_argument('--lrName', required=True, help='the name of Ligand-Receptor dataset')
    parser.add_argument('--matPath', required=True, help='the path of expression matrix')
    parser.add_argument('--matNameL', required=True, help='the name of expression matrix(XL) of sender cell type')
    parser.add_argument('--matNameR', required=True, help='the name of expression matrix(XR) of sender cell type')
    parser.add_argument('--K', required=False, default=2, type=int, help='the number of CSCPs. default=2')
    parser.add_argument('--outPath', required=True, type=str, help='the path to save result')
    ...
    lrName = opt.lrName
    matPath = opt.matPath
    matNameL = opt.matNameL
    matNameR = opt.matNameR
    K = opt.K
    outPath = opt.outPath

    ## load LR
    # lrName = 'PairsLigRec_simple_mouse.txt'
    LRTable = pd.read_csv(os.path.join(lrPath, lrName), sep='\t')

    ## load expression matrix(XL) of sender cell type. 'X' represents matrix, 'G' represents genes.
    # matNameL = 'mat_F002_Endo'
    XL_ori = pd.read_csv(os.path.join(matPath, matNameL), sep=',', index_col=0)

    ## load expression matrix(XR) of receiver cell type. 'X' represents matrix, 'G' represents genes.
    # matNameR = 'mat_F002_Micro'
    XR_ori = pd.read_csv(os.path.join(matPath, matNameR), sep=',', index_col=0)
    ...
    cellSubCrossTalkPairsLabel.to_csv(os.path.join(outPath,'cellSubCrossTalkPairs'))
